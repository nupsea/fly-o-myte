...
class TestDecisionMatrix:
    def test_book_now_on_urgency(self):
        ...
        result = compute(snaps, current_true_cost=1400, days_to_departure=5,
    ...
    def test_book_now_low_signal_rising_trend(self):
        ...
        result = compute(snaps, current_true_cost=1450, days_to_departure=60,
    ...
    def test_book_now_low_signal_short_lead(self):
        ...
        result = compute(snaps, current_true_cost=1260, days_to_departure=30,
                         price_level_signal="LOW")
        assert result.decision == "book_now"
    ...
    def test_monitor_low_signal_long_lead(self):
        ...
        result = compute(snaps, current_true_cost=1310, days_to_departure=90,
                         price_level_signal="LOW")
    ...
    def test_wait_high_signal_falling(self):
        ...
        result = compute(snaps, current_true_cost=1510, days_to_departure=45,
    ...
    def test_wait_falling_no_holiday(self):
        ...
        result = compute(snaps, current_true_cost=1400, days_to_departure=45,
                         price_level_signal="TYPICAL", school_holiday_context=None)
        assert result.decision == "wait"

    def test_wait_holiday_high_price_flexible(self):
        """School holiday + high price + days > 60 + flexible dates → WAIT (priority 8)."""
        holiday = HolidayContext(label="Mid-year holidays", overlap_days=5, is_fully_within=False)
        snaps = make_snapshots([1500, 1510, 1520, 1530, 1540])
        result = compute(snaps, current_true_cost=1540, days_to_departure=90,
                         price_level_signal="HIGH", school_holiday_context=holiday)
        assert result.decision == "wait"
    ...
    def test_monitor_zero_snapshots(self):
        ...
        result = compute(snaps, current_true_cost=1400, days_to_departure=60,
...
class TestRegretRisk:
    def test_low_regret_on_strong_book_signal(self):
        ...
        snaps = make_snapshots([1200, 1250, 1310, 1380, 1450])
        result = compute(snaps, current_true_cost=1450, days_to_departure=6,
                         price_level_signal="LOW")
    ...
    def test_regret_fields_are_non_negative(self):
        ...
        result = compute(snaps, current_true_cost=1498, days_to_departure=30,
                         price_level_signal="TYPICAL")
...
class TestRationale:
    def test_rationale_mentions_price_vs_average(self):
        snaps = make_snapshots([1400, 1410, 1395, 1405, 1400])
        result = compute(snaps, current_true_cost=1400, days_to_departure=30,
                         price_level_signal="TYPICAL")
        assert "$" in result.rationale
        assert "average" in result.rationale.lower()

    def test_rationale_mentions_holiday(self):
        holiday = HolidayContext(label="Autumn holidays", overlap_days=3, is_fully_within=True)
        snaps = make_snapshots([1500, 1510, 1520, 1530, 1540])
        result = compute(snaps, current_true_cost=1540, days_to_departure=30,
                         price_level_signal="HIGH", school_holiday_context=holiday)
    ...
    def test_rationale_mentions_trend_direction(self):
        ...
        result = compute(snaps, current_true_cost=1600, days_to_departure=60,
                         price_level_signal="HIGH")
        assert "rising" in result.rationale.lower() or "falling" in result.rationale.lower()
