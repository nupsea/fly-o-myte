"""
Configuration management for Travo.

Two-layer config:
  1. ~/.travo/config.yaml  — family profile, preferences, API keys
...
TRAVO_DIR = Path(os.environ.get("TRAVO_HOME", "~/.travo")).expanduser()
DEFAULT_CONFIG_PATH = TRAVO_DIR / "config.yaml"
DEFAULT_DB_PATH = TRAVO_DIR / "travo.db"
DEFAULT_ANALYTICS_DIR = TRAVO_DIR / "analytics"
DEFAULT_LOG_PATH = TRAVO_DIR / "travo.log"
...
class Settings(BaseSettings):
    ...
    travo_db_path: str = str(DEFAULT_DB_PATH)
    travo_analytics_dir: str = str(DEFAULT_ANALYTICS_DIR)
    travo_config_path: str = str(DEFAULT_CONFIG_PATH)
    travo_log_path: str = str(DEFAULT_LOG_PATH)
    ...
    def db_path(self) -> Path:
        return Path(self.travo_db_path).expanduser()

    @property
    def analytics_dir(self) -> Path:
        return Path(self.travo_analytics_dir).expanduser()

    @property
    def config_path(self) -> Path:
        return Path(self.travo_config_path).expanduser()

    @property
    def log_path(self) -> Path:
        return Path(self.travo_log_path).expanduser()
...
def load_family_profile(config_path: Path | None = None) -> FamilyProfile:
    """
    Load family profile from ~/.travo/config.yaml.
