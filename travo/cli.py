"""
Travo CLI — all commands defined here.

Entry point: `travo` (registered via pyproject.toml [project.scripts])
...
def _get_engine():
    from travo.db.sqlite import create_db_engine, create_tables
...
def _get_profile():
    from travo.config import load_family_profile
    return load_family_profile()

def _get_pm():
    from travo.tracker import build_plugin_manager_from_settings
...
def version_callback(value: bool) -> None:
    if value:
        console.print(f"travo {__version__}")
        raise typer.Exit()
...
@app.command()
def setup() -> None:
    """First-run configuration wizard. Creates ~/.travo/config.yaml."""
    import yaml
    from travo.config import TRAVO_DIR, DEFAULT_CONFIG_PATH

    TRAVO_DIR.mkdir(parents=True, exist_ok=True)

    console.print("\n[bold]Travo Setup[/bold] — family travel advisor\n")
    console.print("This wizard creates [dim]~/.travo/config.yaml[/dim] with your family profile.\n")

    existing: dict = {}
    if DEFAULT_CONFIG_PATH.exists():
        with DEFAULT_CONFIG_PATH.open() as f:
            existing = yaml.safe_load(f) or {}
        console.print("[yellow]Existing config found — press Enter to keep current values.[/yellow]\n")

    fam = existing.get("family", {})

    adults = int(Prompt.ask("Number of adults", default=str(fam.get("adults", 2))))
    origin = Prompt.ask("Origin airport (IATA)", default=fam.get("origin_airport", "BNE")).upper()
    state = Prompt.ask("State (QLD/NSW/VIC/WA/SA/TAS/NT/ACT)", default=fam.get("state", "QLD")).upper()
    school_type = Prompt.ask("School type (state/independent/catholic)", default=fam.get("school_type", "state"))
    bags = int(Prompt.ask("Checked bags per person", default=str(fam.get("bags_per_person", 1))))
    max_stops = int(Prompt.ask("Max stops (0=nonstop, 1=one stop)", default=str(fam.get("max_stops", 1))))

    children_raw = fam.get("children", [])
    console.print(f"\nCurrently {len(children_raw)} child(ren) in profile.")
    n_children = int(Prompt.ask("Number of children (0–6)", default=str(len(children_raw))))
    ...
    for i in range(n_children):
        ...
        dob = Prompt.ask(f"Child {i + 1} date of birth (YYYY-MM-DD)", default=existing_child.get("dob", ""))
        children.append({"name": name, "dob": dob})

    budget = fam.get("budget_threshold_aud")
    budget_str = Prompt.ask("Budget threshold AUD (optional, press Enter to skip)", default=str(budget) if budget else "")
    ...
    console.print("  2. Run [bold]travo watch BNE SYD 2026-07-20 2026-07-27[/bold] to start tracking a trip")
...
@app.command()
def scout(
    origin: str = typer.Argument(..., help="Origin IATA code (e.g. BNE)"),
    destination: str = typer.Argument(..., help="Destination IATA code (e.g. SYD)"),
    month: Optional[str] = typer.Option(None, help="Month to scout, e.g. jul-2026"),
    depart: Optional[str] = typer.Option(None, help="Specific departure date YYYY-MM-DD"),
    ret: Optional[str] = typer.Option(None, "--return", help="Specific return date YYYY-MM-DD"),
    ...
    from travo.scout import scout_month, scout_flex
    from travo.calendar import get_calendar
    from travo.fees import get_airline_db
    from travo.display import console
    ...
    if month:
        try:
            ...
            err_console.print(f"[red]Invalid month format '{month}'. Use e.g. jul-2026[/red]")
            raise typer.Exit(1)

        console.print(f"\nScouting {origin} → {destination} for {month}...\n")
        results = scout_month(pm, profile, airline_db, cal, origin, destination, year_num, month_num, trip_length)
    elif depart and ret:
        d = date.fromisoformat(depart)
        r = date.fromisoformat(ret)
        if flex:
            results = scout_flex(pm, profile, airline_db, cal, origin, destination, d, r, flex)
        else:
            results = scout_flex(pm, profile, airline_db, cal, origin, destination, d, r, 0)
    else:
    ...
    for r in results[:15]:
        ...
        table.add_row(
            ...
            str(r.stops),
            r.departure_time,
            f"[{holiday_style}]{holiday_text}[/{holiday_style}]" if holiday_text else "—",
        )
...
def watch(
    ...
    return_date: Optional[str] = typer.Argument(None),
    label: str = typer.Option("", help="Friendly name for the trip"),
    adults: Optional[int] = typer.Option(None),
    bags: Optional[int] = typer.Option(None, "--bags"),
    max_stops: Optional[int] = typer.Option(None),
    alert_email: Optional[str] = typer.Option(None),
    alert_threshold: Optional[float] = typer.Option(None),
) -> None:
    """Start tracking a trip and trigger an immediate price fetch."""
    from travo.db.sqlite import Trip, get_session, insert_trip, list_all_trips
    from travo.tracker import poll_trip
    ...
    with get_session(engine) as session:
        ...
        if snap:
            console.print(f"Initial price: [bold]${snap.true_family_cost:,.0f} AUD[/bold]")
        else:
...
@app.command()
def status(
    all_trips: bool = typer.Option(False, "--all", "-a", help="Show all trips including paused"),
) -> None:
    """Morning digest — actionable trips only (or all with --all)."""
    from travo.db.sqlite import get_session, list_all_trips, list_active_trips, get_latest_recommendation
    from travo.display import print_status_digest
...
def check(
    ...
    from travo.db.sqlite import (
        get_session, get_trip, get_latest_recommendation, get_latest_snapshot
    )
    from travo.true_cost import TrueCostBreakdown
    from travo.display import print_trip_detail
    ...
    if not rec or not snap:
        console.print(f"No data yet for trip #{trip_id}. Run [bold]travo refresh {trip_id}[/bold].")
        return
...
@app.command()
def compare(
    trip_ids: list[int] = typer.Argument(..., help="2–3 trip IDs to compare"),
) -> None:
    """Side-by-side comparison of 2–3 trips."""
    from travo.db.sqlite import get_session, get_trip, get_latest_recommendation, get_latest_snapshot
    from travo.display import print_compare_table
...
def history(
    ...
    from travo.db.sqlite import get_session, get_trip, get_snapshots_for_trip
    from travo.display import print_price_history
...
def refresh(
    ...
    from travo.db.sqlite import get_session, get_trip
    from travo.tracker import poll_trip
...
def poll(
    ...
    from travo.db.sqlite import get_session, list_active_trips
    from travo.tracker import poll_all_active
...
def insight(
    ...
    from travo.db.sqlite import get_session, get_trip, get_latest_recommendation, get_latest_snapshot
    from travo.insights import generate_insight
    ...
    depart = __import__("datetime").date.fromisoformat(trip.depart_date)
    ...
    if result:
        ...
        console.print(f"Price impact: {result.price_impact}  |  Event type: {result.event_type}  |  Confidence: {result.confidence:.0%}")
    else:
...
def analytics(
    ...
    from travo.analytics import rebuild_all
    ...
    if rebuild:
        ...
        console.print("Analytics summary not yet implemented. Use --rebuild to rebuild Parquet store.")
...
def pause(trip_id: int = typer.Argument(...)) -> None:
    ...
    from travo.db.sqlite import get_session, set_trip_active
...
def remove(
    ...
    from travo.db.sqlite import get_session, get_trip, get_snapshots_for_trip, delete_trip
...
def data_version() -> None:
    ...
    from travo.fees import get_airline_db
    from travo.display import print_data_version
