#!/bin/bash
# Ralph — Long-running AI agent loop for Fly-O-Myte
# Usage: ./ralph.sh [--tool claude|amp] [max_iterations]
#
# With Claude Code:
#   ./ralph.sh --tool claude 5
#
# Runs the agent loop until all prd.json stories have passes:true,
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if [[ "$TOOL" == "amp" ]]; then
    ...
    # Claude Code: pipe CLAUDE.md as the prompt, run autonomously
  ...
  if echo "$OUTPUT" | grep -q "BLOCKED: Demo Review Gate"; then
    echo ""
    echo "Ralph is blocked on a demo review gate."
    echo "Review the output above and create the approval file to continue."
    exit 2
  fi
